...
def generate_image(prompt: str) -> dict[str, Any]:
    ...
    try:
        ...
        images_dir = Path.home() / "gemmis_images"
        ...
        filename = images_dir / f"{safe_name}_{timestamp}.jpg"
        ...
        with httpx.Client() as client:
            ...
            if response.status_code == 200:
                ...
                # OMEGA UPGRADE: Generate ASCII art for terminal preview
                ascii_art = image_to_ascii(response.content)
                
                return {
                    "success": True, 
                    "filepath": str(filename),
                    "ascii": ascii_art,
                    "message": f"Image generated! Saved to {filename}. Rendering ASCII preview below..."
                }
            else:
...
TOOLS = [
    {
        ...
        "function": {
            "name": "generate_image",
            "description": "Generera en bild från en text-beskrivning (prompt). Sparas i ~/gemmis_images/",
            "parameters": {
                "type": "object",
                "properties": {
                    "prompt": {
                        "type": "string",
                        "description": "Beskrivning av bilden (på engelska för bäst resultat)"
                    }
                },
                "required": ["prompt"],
            },
